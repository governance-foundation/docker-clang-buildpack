...
#
IMAGE_NAME=${1:-gvfn/clang-buildpack}
IMAGE_CHECK=${2:-clang version}
FLAG_DEBUG=${3:-true}
IP=$(which ip)
if [[ -z $IP ]]; then
    LOCAL_IP="localhost"
...
test_docker_run_contains_clang() {
    printLine "Testing if image has clang"

    printLine "Starting Container"

    OUTPUT=$(docker run --rm ${IMAGE_NAME} bash -c "clang --version")

    if [[ "$OUTPUT" != *"$IMAGE_CHECK"* ]]; then
        printResult "error"
